...
export function ChatInterface() {
    ...
    return (
        <div className="flex h-screen bg-tana-bg overflow-hidden relative">
            ...
            {/* Sidebar */}
            <div className={`fixed md:relative z-50 w-64 bg-tana-card/30 border-r border-tana-border flex flex-col h-full transition-transform duration-300 transform ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 bg-tana-bg md:bg-transparent`}>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    ...
                    {['Project Planning', 'Meeting Notes Analysis', 'Content Ideas'].map((thread, i) => (
                        <button key={i} className="w-full text-left px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-white/5 hover:text-white transition-colors truncate">
                            {thread}
                        </button>
                    ))}
                </div>
            ...
            <div className="flex-1 flex flex-col relative w-full">
                ...
                <div className="h-16 border-b border-tana-border flex items-center justify-between px-4 md:px-6 bg-tana-bg/80 backdrop-blur-md z-10">
                    <div className="flex items-center space-x-2">
                        <button onClick={() => setMobileSidebarOpen(true)} className="md:hidden text-gray-400 mr-2">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth">
                    {messages.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-2xl ${msg.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-tana-card border border-tana-border text-gray-200'} rounded-2xl px-6 py-4 shadow-lg`}>
                                ...
                                {msg.nodes && (
                                    <div className="mt-4 space-y-3">
                                        {msg.nodes.map(node => (
                                            <div key={node.id} className="bg-black/40 rounded-xl p-4 border border-tana-border hover:border-indigo-500/50 transition-colors cursor-pointer group">
                                                <div className="flex items-center space-x-2 mb-3">
                                                    <span className="w-2 h-2 rounded-full bg-yellow-500"></span>
                                                    <span className="font-bold text-white group-hover:text-indigo-400 transition-colors">{node.title}</span>
                ...
                <div className="p-6 bg-gradient-to-t from-tana-bg via-tana-bg to-transparent">
                    ...
                    <div className="text-center mt-2">
                        <p className="text-[10px] text-tana-muted">AI can make mistakes. Please verify important information.</p>
                    </div>
                </div>
            </div>
        </div>
